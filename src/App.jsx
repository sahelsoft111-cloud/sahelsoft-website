...
function PhoneFrame({ shot, featured = false }) {
  return (
    <a href={shot.src} target="_blank" rel="noreferrer" className={`group block rounded-[2rem] border-[8px] border-slate-950 bg-slate-950 shadow-premium transition duration-300 hover:-translate-y-2 ${featured ? "w-[248px] sm:w-[290px]" : "w-[172px] sm:w-[190px]"}`}>
      <img src={shot.src} alt={shot.alt} className="aspect-[9/17] w-full rounded-[1.25rem] bg-white object-contain" loading={featured ? "eager" : "lazy"} decoding="async" />
    </a>
  );
}
  ...
  return (
    <div className="relative mx-auto flex min-h-[430px] w-full max-w-[640px] items-center justify-center overflow-hidden rounded-3xl bg-gradient-to-br from-blue-50 via-white to-sky-50 px-6 py-8 shadow-soft">
      <div className="absolute left-8 top-8 hidden rotate-[-8deg] opacity-70 blur-[0.1px] md:block">
        <PhoneFrame shot={sugucash.screenshots[(active + 1) % sugucash.screenshots.length]} />
      </div>
      <div className="relative z-10">
        <PhoneFrame shot={current} featured />
      </div>
      <div className="absolute bottom-8 right-8 hidden rotate-[8deg] opacity-70 blur-[0.1px] md:block">
        <PhoneFrame shot={sugucash.screenshots[(active + 2) % sugucash.screenshots.length]} />
      </div>
